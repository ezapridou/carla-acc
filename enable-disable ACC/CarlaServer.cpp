// path: Unreal/CarlaUE4/Plugins/Carla/Source/Carla/Server
...
void FCarlaServer::FPimpl::BindActions()
{
      ...
      bool bEnabled) -> R<void>
  {
    ...
    if (!ActorView.IsValid())
    {
      RESPOND_ERROR("unable to set ACC: actor not found");
    }
    auto Vehicle = Cast<ACarlaWheeledVehicle>(ActorView.GetActor());
    if (Vehicle == nullptr)
    {
      RESPOND_ERROR("unable to set ACC: actor does not support ACC");
    }
    auto Controller = Cast<AWheeledVehicleAIController>(Vehicle->GetController());
    if (Controller == nullptr)
    {
      RESPOND_ERROR("unable to set ACC: vehicle controller does not support ACC");
    }
    Controller->SetACC(bEnabled);
    return R<void>::Success();
  };
